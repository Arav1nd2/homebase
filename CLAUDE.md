...
HomeBase: a personal life-management app for a small household, built with Next.js (App Router) + TypeScript, Supabase Postgres via Drizzle ORM, and deployed to Cloudflare Workers via the OpenNext adapter. Auth is Supabase Auth with email one-time codes, delivered through a custom Send Email Hook worker + Resend.
...
**UI shell foundation** (`specs/003-ui-shell-foundation`): the shared kernel every tool screen renders on top of. Tailwind v4 (CSS-first `@theme` config, no `tailwind.config.ts`) and shadcn/ui (`packages/web/components.json`) are configured; `packages/web/styles/globals.css` is the single source for the "Verse Margin" design tokens (type scale, spacing, color — both light and `[data-theme="dark"]`), transcribed verbatim from `DESIGN.md`. `components/ui` holds raw, unopinionated shadcn primitives; `components/shared` holds HomeBase-specific composed patterns (`PageHeader`, `LoadingState`, `EmptyState`, `ErrorState`) — no domain module reimplements these. `providers/app-providers.tsx` composes theme switching (`next-themes`, no visible toggle — OS-detected, with a `?theme=` query-param override for verification only) and TanStack Query scaffolding (`lib/query-client.ts`, `lib/api-client.ts`); it deliberately has no toast/notification provider, since DESIGN.md rules out visual toast chrome everywhere. `/styleguide` is the example screen proving the shell renders correctly — no tool-specific content. No `domains/` folder exists yet.
